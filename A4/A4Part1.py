...
def extractMainLobe(window, M):
    ...
    # take the FFT size (N) to be 8 times the window length (N = 8M)
    # (For this part, N need not be a power of 2).
    N = 8*M
    X = fft(w, N)

        #If you find it difficult to concatenate the two halves of the main lobe,
    #  you can first center the spectrum using fftshift()
    shift = fftshift(X)
    mX = 20 * np.log10(np.abs(shift))

    maxIndex = np.argmax(mX) #get absolute max

    def findLocalMinima(arr):
        return np.r_[True, arr[1:] < arr[:-1]] & np.r_[arr[:-1] < arr[1:], True]

    def find_nearest(array,value):                   
        idx = (np.abs(array-value)).argmin()
        return array[idx]

    #from max, advance until first minimum, both sides
    localMinimaIndexes = np.where(findLocalMinima(mX)==True)[0]
    leftSideMinimum = find_nearest(localMinimaIndexes, maxIndex-1)
    rightSideMinimum = find_nearest(localMinimaIndexes, maxIndex+1)
    
    mainLobe = np.array(mX[leftSideMinimum:rightSideMinimum+1])
    return mainLobe
