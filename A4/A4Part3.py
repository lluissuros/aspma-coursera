...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    # 2 compute two energy values for each frequency band specified

    lowFreqBin = int(np.ceil(N/float(fs) * 3000.0))
    highFreqBin = int(np.ceil(N/float(fs) * 10000.0))

    lowFreqBand = np.transpose(np.transpose(mX)[1:lowFreqBin])
    highFreqBand = np.transpose(np.transpose(mX)[lowFreqBin :highFreqBin])

    #low : from 0 to 3000 hz
    # high: from 3000 to 10000
    # #value of the energy envelope of a band can be computed as the sum of squared values of all the frequency coefficients in that band
#energy in D

    def bandEnergy(band):
        linear = 10 ** (band / 20)
        energyLineal = np.sum(pow(np.abs(linear), 2), axis=1)
        energyDB = 10 * np.log10(energyLineal)
        return energyDB

    lowFreqEnergy = bandEnergy(lowFreqBand)
    highFreqEnergy = bandEnergy(highFreqBand)

    engEnv = np.array([lowFreqEnergy, highFreqEnergy])
    ...
    plt.subplot(212)
    plt.plot(lowFreqEnergy, color="blue")
    plt.plot(highFreqEnergy, color="red")
    plt.title('high and low bandsz')
    plt.autoscale(tight=True)
