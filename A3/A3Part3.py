...
def testRealEven(x):
    ...
    dftbuffer[:int(hM1)] = x[int(hM2):]
    dftbuffer[int(-hM2):] = x[:int(hM2)]
    X = fft(dftbuffer)

    isRealEven =  not any(np.imag(X) > 1.0e-6)

    isSymmetric = all(np.real(X[1:hM1]) == np.real(X[:hM2:-1]))
    has0Phase = all(np.imag(X) < 1.0e-6)
    isRealEven = isSymmetric and has0Phase
