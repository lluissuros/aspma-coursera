...
def optimalZeropad(x, fs, f):
    ...
    ## Your code here
    M = x.size
    samplesPerPeriod = float(fs/f)
    periodsInFile = x.size/samplesPerPeriod
    N = np.ceil(periodsInFile) * samplesPerPeriod #lenght with zero padding 
    
    withZeroPadding = np.zeros(int(N))
    withZeroPadding[:x.size]= x  

    X = fft(withZeroPadding)
    mX = 20 * np.log10(np.abs(X[: int(N/2) + 1]))
    return mX
